...
function setup(expiring:unknown[]=[],expired:unknown[]=[]){
 ...
 const empty={};
 const service=new BusinessService({readyState:1} as never,cards as never,empty as never,memberships as never,empty as never,empty as never,empty as never,empty as never,empty as never,notices as never,audits as never,empty as never,empty as never,empty as never);
 return{service,cards,memberships,notices,audits};
}

describe('daily membership automation',()=>{
 const now=new Date('2026-09-21T00:00:00.000Z');
 it('creates an idempotent expiration reminder',async()=>{const membership={membership_public_id:'MEM-1',customer_id:'CUS-1',tier_code:'GOLD',ends_at:new Date('2026-10-01T00:00:00.000Z')};const{service,notices}=setup([membership],[]);const result=await service.runDailyAutomation(now);expect(result).toEqual(expect.objectContaining({reminders_created:1,memberships_expired:0}));expect(notices.updateOne).toHaveBeenCalledWith({event_key:'MEMBERSHIP_EXPIRING:MEM-1:2026-10-01T00:00:00.000Z'},expect.objectContaining({$setOnInsert:expect.objectContaining({recipient_id:'CUS-1'})}),{upsert:true})});
