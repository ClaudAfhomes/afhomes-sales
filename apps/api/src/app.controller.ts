import {Body,Controller,Get,Post} from '@nestjs/common'; import {ApiBearerAuth,ApiTags} from '@nestjs/swagger'; import {AuthService} from './auth.service'; import {BusinessService} from './business.service'; import {CurrentUser,Public,Roles,type AuthenticatedUser} from './auth.decorators'; import {ActivateCardDto,CreateTicketDto,CreateTransactionDto,LoginDto,MemberLookupDto,ReverseTransactionDto,SellCardDto} from './dto';
...
@Controller() export class AppController{constructor(private auth:AuthService,private business:BusinessService){}
 ...
 @ApiBearerAuth() @Roles('CUSTOMER') @ApiTags('cards') @Post('api/v1/cards/activate') activate(@Body() d:ActivateCardDto,@CurrentUser() user:AuthenticatedUser){return this.business.activate(d,user.sub)}
 @ApiBearerAuth() @Roles(...STAFF) @ApiTags('members') @Post('api/v1/members/lookup') lookup(@Body() d:MemberLookupDto,@CurrentUser() user:AuthenticatedUser){return this.business.lookupMember(d,user.sub)}
 @ApiBearerAuth() @Roles('SUPER_ADMIN','ADMIN','MANAGER','CASHIER') @ApiTags('transactions') @Post('api/v1/transactions') transact(@Body() d:CreateTransactionDto,@CurrentUser() user:AuthenticatedUser){return this.business.transact(d,user.sub)}
