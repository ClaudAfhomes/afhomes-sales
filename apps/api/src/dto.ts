...
export class MemberLookupDto{@IsIn(['dynamic_qr_token','printed_qr_token','nfc_token','member_code','card_public_id']) method!:'dynamic_qr_token'|'printed_qr_token'|'nfc_token'|'member_code'|'card_public_id';@IsString() value!:string}
export class ItemDto{@IsString() product_code!:string;@IsOptional() @IsString() description?:string;@IsInt() @IsPositive() quantity!:number;@IsOptional() @IsInt() @Min(0) unit_amount_minor?:number}
export class CreateTransactionDto{@IsString() idempotency_key!:string;@IsString() branch_id!:string;@IsString() membership_id!:string;@IsOptional() @IsIn(['dynamic_qr_token','printed_qr_token','nfc_token','member_code','card_public_id']) card_lookup_method?:'dynamic_qr_token'|'printed_qr_token'|'nfc_token'|'member_code'|'card_public_id';@IsOptional() @IsString() card_identifier?:string;@IsIn(['SALE','REDEEM']) type!:string;@IsString() currency_code!:string;@IsInt() @Min(0) points_redeemed=0;@IsArray() @ArrayMinSize(1) @ValidateNested({each:true}) @Type(()=>ItemDto) items!:ItemDto[]}
export class UpsertProductDto{@IsString() product_code!:string;@IsString() @Length(2,160) name!:string;@IsIn(['PRODUCT','SERVICE']) kind!:'PRODUCT'|'SERVICE';@IsString() currency_code!:string;@IsInt() @Min(0) unit_amount_minor!:number;@IsOptional() @IsArray() @IsString({each:true}) branch_ids?:string[];@IsOptional() @IsBoolean() is_active?:boolean}
export class UpsertTierDto{@IsString() tier_code!:string;@IsString() @Length(2,100) name!:string;@IsInt() @IsPositive() rank!:number;@IsInt() @IsPositive() duration_value!:number;@IsIn(['DAY','MONTH','YEAR']) duration_unit!:'DAY'|'MONTH'|'YEAR';@IsOptional() @IsBoolean() is_active?:boolean}
